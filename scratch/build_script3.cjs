...
const remaining = src.substring(rppStart);
let depth = 0;
let endIndex = -1;
...
const rppEnd = rppStart + endIndex;
let rppBlock = src.substring(rppStart, rppEnd);
// Find the first "return (" in rppBlock
const firstReturn = rppBlock.indexOf('return (');
rppBlock = rppBlock.substring(firstReturn); // exclude "if (isRppMode) {"

const statesStart = src.indexOf('export default function Show');
const statesEnd = rppStart;
...
statesBlock = statesBlock.replace(/useState<string>\(school_setting\('app_name', 'Nama Sekolah'\)\)/g, 'useState<string>(school_name)');
...
// Make sure to close the Show function because we stripped the if block brace
