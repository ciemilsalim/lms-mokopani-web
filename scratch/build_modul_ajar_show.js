...
const rppReturnEnd = src.indexOf('        );', rppReturnStart) + 10;
let rppBlock = src.substring(rppReturnStart, rppReturnEnd);

// 2. Extract states needed
// Find where states are declared (after 'export default function Show')
...
statesBlock = statesBlock.replace(/const \[my_reflection.*?\] = useState.*?;/g, '');
statesBlock = statesBlock.replace(/const \[all_reflections.*?\] = useState.*?;/g, '');
statesBlock = statesBlock.replace(/const \[comments.*?\] = useState.*?;/g, '');
statesBlock = statesBlock.replace(/const \[is_completed.*?\] = useState.*?;/g, '');
statesBlock = statesBlock.replace(/const \[readiness_status.*?\] = useState.*?;/g, '');
statesBlock = statesBlock.replace(/const \[showDeleteConfirm.*?\] = useState.*?;/g, '');
...
statesBlock = statesBlock.replace(/export default function Show[\s\S]*?\{/, newInterface);
...
statesBlock = statesBlock.replace(/useState<string>\(isStructuredDataMaterial \? "LEMBAR KERJA PESERTA DIDIK: PENGELOMPOKAN DATA" : "LEMBAR KERJA PESERTA DIDIK \(LKPD\)"\)/, 'useState<string>(savedConfig.lkpdTitle || (isStructuredDataMaterial ? "LEMBAR KERJA PESERTA DIDIK: PENGELOMPOKAN DATA" : "LEMBAR KERJA PESERTA DIDIK (LKPD)"))');

// For LKPD steps
statesBlock = statesBlock.replace(/useState<string>\([\s\S]*?LKPD[\s\S]*?\]\);/, useState<string>(savedConfig.lkpdLangkah || (isStructuredDataMaterial ? "- Diskusikan...\\n- Catat...\\n- Buat..." : "- Amati stimulus...\\n- Diskusikan masalah...\\n- Buat rancangan solusi...\\n- Presentasikan hasil...")););

// For the rest string states, we just inject savedConfig manually later in the script or do it simply.
...
// Modify rppBlock
// Remove 'isRppMode && ' or just use the block
// Replace 'Kembali ke Detail Materi' button with 'Kembali' back to list
rppBlock = rppBlock.replace(/Kembali ke Detail Materi/g, 'Kembali ke Daftar Modul Ajar');
rppBlock = rppBlock.replace(/setIsRppMode\(false\)/g, 'window.history.back()');

// Add "Simpan Perubahan" button next to Cetak RPP
const printBtnHtml = \<button onClick={handlePrint}\;
const saveBtnHtml = \
    <button onClick={handleSaveConfig} disabled={isSaving} className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-indigo-700 shadow-md">
...
console.log('Successfully built modul-ajar/show.tsx');
