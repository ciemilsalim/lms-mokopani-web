...
export default function P5Edit({ project, classes, dimensi, sipada_projects }: P5EditProps) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        cocurricular_id: project.cocurricular_id ? project.cocurricular_id.toString() : '',
    });
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: project.judul, href: route('p5.show', project.id) },
        { title: 'Edit', href: '#' },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${project.judul} – Projek P5`} />

            <div className="flex h-full flex-1 flex-col gap-6 max-w-3xl mx-auto w-full min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <button 
                        onClick={() => window.history.length > 1 ? window.history.back() : router.visit(route('p5.show', project.id))}
                    ...
                    >
                        <ChevronLeft className="h-4 w-4" /> Kembali ke Detail Projek
                    </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        {sipada_projects && sipada_projects.length > 0 && (
                            <div className="space-y-2 p-4 border border-primary/20 bg-primary/5 rounded-lg">
                                <label className="text-xs font-semibold text-primary flex items-center gap-2">
                                    <Heart className="h-4 w-4" /> Import Data dari Kokurikuler (Opsional)
                                </label>
                                <select 
                    ...
                    <div className="flex flex-col sm:flex-row justify-end gap-3">
                        <button type="button" onClick={() => window.history.length > 1 ? window.history.back() : router.visit(route('p5.show', project.id))} className="w-full sm:w-auto px-6 py-2.5 text-sm font-medium text-muted-foreground hover:text-foreground transition text-center">Batal</button>
